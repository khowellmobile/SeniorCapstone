 ...
 *	@param		p_Amount				FLOAT				Amount of transaction
...
/*
 *	Name:		spAccounts_Add
 *	Purpose: 	Adds a line in the ScribeDb Accounts table
 *  
 *	@param		p_AccountTypeID			INT		AccountTypeID of the account in the AccountTypes table
 *	@param		p_TransactionID			INT		TransactionID of the transaction in the TransactionsID table
 *	@param		p_isDeleted				INT		If the account is deleted or not
*/
CREATE PROCEDURE spAccounts_Add
...
/*
 *	Name:		spAccounts_Update
 *	Purpose: 	Updates a line in the ScribeDb Accounts table
 *  
 * 	@param		p_AccountID				INT		AccountID of the account in the Accounts table
 *	@param		p_AccountTypeID			INT		AccountTypeID of the account in the AccountTypes table
 *	@param		p_TransactionID			INT		TransactionID of the transaction in the TransactionsID table
 *	@param		p_isDeleted				INT		If the account is deleted or not
*/
CREATE PROCEDURE spAccounts_Update
	(IN	 p_AccountID			INT	
		,p_AccountTypeID		INT
		,p_TransactionID		INT
		,p_isDeleted			INT)
...
BEGIN
	...
	IF(!EXISTS(SELECT * FROM Accounts WHERE AccountID = p_AccountID)) THEN
		SIGNAL ID_Doesnt_Exist;
	ELSE
		SELECT		
			 p_AccountID			= IFNULL(p_AccountID		, p_AccountID)			
			,p_AccountTypeID		= IFNULL(p_AccountTypeID	, p_AccountTypeID)	
			,p_TransactionID		= IFNULL(p_TransactionID	, p_TransactionID)	
			,p_isDeleted			= IFNULL(p_isDeleted		, p_isDeleted)			
		FROM Accounts
...
BEGIN
	...
	ELSE
		SELECT		
			 p_AccountTypeID		= IFNULL(p_AccountTypeID		, p_AccountTypeID)
			,p_AccountTypeName		= IFNULL(p_AccountTypeName		, p_AccountTypeName)
			,p_Description			= IFNULL(p_Description			, p_Description)	
			,p_isMain				= IFNULL(p_isMain				, p_isMain)		
			,p_isDeleted			= IFNULL(p_isDeleted			, p_isDeleted)	
		FROM AccountTypes
		WHERE AccountTypeID = p_AccountTypeID;

		UPDATE AccountTypes SET
			 p_AccountTypeName	 =	p_AccountTypeName	
