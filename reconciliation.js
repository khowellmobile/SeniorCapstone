$(document).ready(function () {
    ...
    if (accountResponseData == "0 results[]") {
        alert("This client does not have any bank accounts. Please add one");
    }
    ...
    $("#sumbitButton").click(function(){
        ...
        for (var i = 0; i < jsonData.length; i++) {
            pdata += '{"TransactionID":"' + jsonData[i].TransactionID + '","row' + i + 'reconciled":"' + $('#row-' + i + '-reconciled').is(":checked") + 
                '", "Amount":"' + jsonData[i].Amount + '", "Account":"' + jsonData[i].AccountTypeName + '"},';
        }
    ...
    $("#ReconInfoSubmit").click(function(){
        ...
        if (!tableLoaded) {
            ...
            for(var i = 0; i < jsonData.length; i++) {
                if (jsonData[i].Amount < 0) {
                    debit = (Math.abs(jsonData[i].Amount)).toFixed(2);
                    credit = "";
                } else {
        ...
        if (Number(currentDiffBalance) < 0) {
            $("#DiffAccountTotal").text("(" + currentDiffBalance.toFixed(2).slice(1) + ")");
        } else {
    ...
    $("tbody").on('click', 'tr', function(){
        ...
        } else if (Number(newDiff.toFixed(2)) == 0 ) {
            $("#DiffAccountTotal").text("0.00");
        } else {
